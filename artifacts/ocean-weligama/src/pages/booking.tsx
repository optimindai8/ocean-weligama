...
import {
  ChevronRight, Check, Users, CreditCard, Sparkles, ShieldCheck, Waves,
  ArrowRight, Award, Ship, Compass, Utensils, Bike, Plane, Calendar,
  Clock, ChevronLeft
} from "lucide-react";
...
// ─────────────────────────────────────────────────────────────
// Zod schema – airport fields are optional strings by default;
// flight details are required only when pickup is selected.
// ─────────────────────────────────────────────────────────────
const guestSchema = z
  .object({
  ...
  .superRefine((data, ctx) => {
    if (data.airportPickup) {
      if (!data.flightNumber || data.flightNumber.trim() === "") {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Flight number is required for airport pickup", path: ["flightNumber"] });
      }
      if (!data.flightDate || data.flightDate.trim() === "") {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Arrival date is required", path: ["flightDate"] });
      }
      if (!data.flightTime || data.flightTime.trim() === "") {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Arrival time is required", path: ["flightTime"] });
      }
    }
  });
...
// ─────────────────────────────────────────────────────────────
// Airport transfer pricing helper
// ─────────────────────────────────────────────────────────────
function getAirportPrices(guestCount: number) {
  const isLarge = guestCount >= 4;
  return {
    pickup: isLarge ? 100 : 75,
    drop: isLarge ? 100 : 65,
    pickupLabel: isLarge ? "€100 (4+ pax)" : "€75 (up to 3 pax)",
    dropLabel: isLarge ? "€100 (4+ pax)" : "€65 (up to 3 pax)",
  };
}
...
export default function BookingPage() {
  ...
  const airportPrices = getAirportPrices(guestCount);

  // Total airport transfer amount
  const airportTransferTotal =
    (watchPickup ? airportPrices.pickup : 0) + (watchDrop ? airportPrices.drop : 0);
  ...
  async function onSubmit(data: GuestForm) {
    ...
    try {
      ...
      // Prepend airport transfer details
      const transferLines: string[] = [];
      if (data.airportPickup) {
        transferLines.push(
          `[Airport Pick-up: ${airportPrices.pickupLabel} — Pay on Arrival]`,
          `  Flight Number: ${data.flightNumber || "—"}`,
          `  Arrival Date: ${data.flightDate || "—"}`,
          `  Arrival Time: ${data.flightTime || "—"}`,
          `  Bringing Surfboard: ${data.bringingSurfboard ? "Yes" : "No"}`
        );
      }
      if (data.airportDrop) {
        transferLines.push(
          `[Airport Drop: ${airportPrices.dropLabel} — Pay on Arrival]`
        );
      }
      if (transferLines.length > 0) {
        finalSpecialRequests = transferLines.join("\n") + (finalSpecialRequests ? "\n" + finalSpecialRequests : "");
      }

      // Prepend pay-on-arrival add-ons
  ...
  const STEPS = [
    ...
    { n: 3, label: "Airport Transfer", icon: Plane },
    { n: 4, label: "Confirm & Pay", icon: CreditCard },
  ];
      ...
      <main className="flex-1 py-16 bg-muted/20">
        <div className="container mx-auto px-4 max-w-7xl">
          <AnimatePresence mode="wait">
            ...
            {/* ── STEP 3: Airport Transfer (Optional) ──────────── */}
            {step === 3 && (
              <motion.div
                key="step3"
                initial={{ opacity: 0, x: 20 }}
              ...
              >
                ...
                <header className="text-center space-y-4">
                  ...
                  <h1 className="text-4xl md:text-6xl font-serif font-bold text-foreground italic">Airport Transfer</h1>
                  <p className="text-muted-foreground text-lg max-w-lg mx-auto">
                    Let us handle your journey to and from Bandaranaike International Airport — seamlessly and comfortably.
                  </p>
                </header>

                <div className="space-y-6">

                  {/* ── Airport Pick-up Card ── */}
                  <div className={`rounded-[2rem] border-2 transition-all duration-500 overflow-hidden shadow-lg ${
                    watchPickup
                    ...
                    <button
                      ...
                      className="w-full text-left p-6 sm:p-8 flex items-start gap-5"
                    >
                      <div className={`mt-1 w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all duration-300 ${
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${
                              watchPickup ? "bg-primary text-white" : "bg-sky-100 text-sky-600"
                            }`}>
                              <Plane className="w-5 h-5" />
                            </div>
                            <div>
                              <p className="text-lg font-serif font-bold text-foreground">Airport Pick-up</p>
                              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">up to 3 people</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className={`text-2xl font-bold ${watchPickup ? "text-primary" : "text-muted-foreground"}`}>
                              {airportPrices.pickup === 75 ? "EUR 75" : "EUR 100"}
                            </p>
                            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Pay on Arrival</p>
                          </div>
                        </div>
                        <p className="text-sm text-muted-foreground leading-relaxed">
                          Transfer from Bandaranaike International Airport to the Surf Camp.{" "}
                          <span className="font-semibold text-foreground/70">Price for up to 3 pax: €75. For 4+ pax: €100.</span>
                        </p>
                      </div>
                    </button>

                    {/* Expanded: Arrival Info + Surfboard */}
                    <AnimatePresence>
                      {watchPickup && (
                        <motion.div
                        ...
                        >
                          <div className="px-6 sm:px-8 pb-8 space-y-8 border-t border-border/40 pt-6">

                            {/* Surfboard query */}
                            <div className="flex items-center justify-between bg-sky-50 rounded-2xl p-5 border border-sky-100">
                              <div>
                                <p className="font-bold text-foreground">Bringing a surfboard?</p>
                                <p className="text-xs text-muted-foreground mt-0.5">We'll arrange extra space in the vehicle</p>
                              </div>
                              <div className="flex gap-3">
                                <button
                                  type="button"
                                  onClick={() => form.setValue("bringingSurfboard", false)}
                                  className={`px-5 py-2 rounded-full text-sm font-bold border-2 transition-all duration-200 ${
                                    watchSurfboard === false
                                ...
                                <button
                                  type="button"
                                  onClick={() => form.setValue("bringingSurfboard", true)}
                                  className={`px-5 py-2 rounded-full text-sm font-bold border-2 transition-all duration-200 ${
                                    watchSurfboard === true
                                      ? "bg-primary border-primary text-white shadow-md"
                                      : "bg-white border-muted text-muted-foreground hover:border-primary/30"
                            ...
                            <div className="space-y-5">
                              <div className="flex items-center gap-2">
                                ...
                                <p className="text-sm font-black uppercase tracking-widest text-foreground">Arrival Information</p>
                              </div>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                                ...
                                <FormField control={form.control} name="flightNumber" render={({ field }) => (
                                  <FormItem className="sm:col-span-1">
                                    <FormLabel className="text-[10px] font-black uppercase tracking-widest ml-1 flex items-center gap-1.5">
                                      <Plane className="w-3 h-3" /> Arrival Flight Number
                                    </FormLabel>
                                    <FormControl>
                                      <Input
                                        placeholder="e.g. QR 654"
                                        className="h-13 rounded-2xl border-muted bg-white focus:bg-white px-5 font-mono"
                                        {...field}
                                      />
                                    </FormControl>
                                    <FormMessage />
                                ...
                                <FormField control={form.control} name="flightDate" render={({ field }) => (
                                  <FormItem className="sm:col-span-1">
                                    <FormLabel className="text-[10px] font-black uppercase tracking-widest ml-1 flex items-center gap-1.5">
                                      <Calendar className="w-3 h-3" /> Arrival Flight Date
                                    </FormLabel>
                                    <FormControl>
                                      <Input
                                        type="date"
                                        ...
                                        className="h-13 rounded-2xl border-muted bg-white focus:bg-white px-5"
                                        {...field}
                                      />
                                    </FormControl>
                                    <FormMessage />
                                ...
                                <FormField control={form.control} name="flightTime" render={({ field }) => (
                                  <FormItem className="sm:col-span-1">
                                    <FormLabel className="text-[10px] font-black uppercase tracking-widest ml-1 flex items-center gap-1.5">
                                      <Clock className="w-3 h-3" /> Arrival Flight Time
                                    </FormLabel>
                                    <FormControl>
                                      <Input
                                        type="time"
                  ...
                  {/* ── Airport Drop Card ── */}
                  <div className={`rounded-[2rem] border-2 transition-all duration-500 shadow-lg ${
                    watchDrop
                      ? "border-primary bg-primary/5 shadow-primary/10"
                      : "border-white bg-white"
                  }`}>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${
                              watchDrop ? "bg-primary text-white" : "bg-emerald-100 text-emerald-600"
                            }`}>
                              <Plane className="w-5 h-5 rotate-90" />
                            </div>
                            <div>
                              <p className="text-lg font-serif font-bold text-foreground">Airport Drop</p>
                              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">up to 3 people</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className={`text-2xl font-bold ${watchDrop ? "text-primary" : "text-muted-foreground"}`}>
                              {airportPrices.drop === 65 ? "EUR 65" : "EUR 100"}
                            </p>
                            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Pay on Arrival</p>
                          </div>
                        </div>
                        <p className="text-sm text-muted-foreground leading-relaxed">
                          ...
                          <span className="font-semibold text-foreground/70">Price for up to 3 pax: €65. For 4+ pax: €100.</span>
                        </p>
                      </div>
                    </button>
                  </div>

                  {/* Pricing breakdown (shown when at least one is selected) */}
                  <AnimatePresence>
                    {(watchPickup || watchDrop) && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 10 }}
                        className="bg-primary/5 border border-primary/15 rounded-2xl p-6 flex items-center justify-between"
                      >
                        <div className="space-y-1">
                          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Transfer Total</p>
                          <p className="text-xs text-muted-foreground">
                            {[watchPickup && `Pick-up (${airportPrices.pickupLabel})`, watchDrop && `Drop (${airportPrices.dropLabel})`]
                              .filter(Boolean)
                              .join(" + ")}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="text-2xl font-bold text-primary">€{airportTransferTotal}</p>
                          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Pay on Arrival</p>
                        </div>
                      </motion.div>
                    )}
                ...
                {/* Navigation */}
                <div className="flex flex-col-reverse sm:flex-row justify-between gap-4 sm:gap-6 pt-4">
                  <Button
                    variant="outline"
                    onClick={() => setStep(2)}
                    className="w-full sm:w-auto rounded-full px-6 sm:px-12 h-14 font-bold border-muted text-muted-foreground hover:bg-muted flex items-center justify-center gap-2"
                  >
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <Button
                      ...
                      className="w-full sm:w-auto rounded-full px-6 sm:px-10 h-14 font-bold border-muted text-muted-foreground hover:bg-muted"
                    >
                    ...
                    <Button
                      ...
                      className="w-full sm:w-auto bg-primary hover:bg-secondary text-white rounded-full px-6 sm:px-12 h-14 font-bold shadow-xl flex items-center justify-center gap-3"
                    >
                      Review & Confirm
                      <ArrowRight className="w-5 h-5" />
                    </Button>
                ...
                <div className="bg-white rounded-[2rem] sm:rounded-[4rem] p-5 sm:p-12 md:p-20 shadow-2xl border border-border/50 space-y-8 md:space-y-12">
                  ...
                  {(watchPickup || watchDrop) && (
                    <div className="border-b border-border pb-8 md:pb-12 space-y-6">
                      ...
                      <div className="space-y-4">
                        {watchPickup && (
                          <div className="bg-sky-50 border border-sky-100 rounded-2xl p-5 space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-right">
                                <p className="font-bold text-primary">€{airportPrices.pickup}</p>
                                <p className="text-[9px] text-muted-foreground uppercase font-black tracking-widest">Pay on Arrival</p>
                              </div>
                            </div>
                            <div className="grid grid-cols-3 gap-3 pt-2 border-t border-sky-100">
                              <div>
                                <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Flight</p>
                                <p className="text-sm font-bold font-mono">{form.getValues("flightNumber") || "—"}</p>
                              </div>
                              <div>
                                <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Date</p>
                                <p className="text-sm font-bold">{form.getValues("flightDate") || "—"}</p>
                              </div>
                              <div>
                                <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Time</p>
                                <p className="text-sm font-bold">{form.getValues("flightTime") || "—"}</p>
                              </div>
                            </div>
                            {watchSurfboard && (
                              <div className="flex items-center gap-2 text-xs text-sky-700 font-bold bg-sky-100 px-3 py-1.5 rounded-full w-fit">
                        ...
                        {watchDrop && (
                          <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-5">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-right">
                                <p className="font-bold text-primary">€{airportPrices.drop}</p>
                                <p className="text-[9px] text-muted-foreground uppercase font-black tracking-widest">Pay on Arrival</p>
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="pt-8 md:pt-12 border-t border-border flex flex-col items-center gap-6 md:gap-8">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      {airportTransferTotal > 0 && (
                        <p className="text-sm text-muted-foreground">
                          + €{airportTransferTotal} airport transfer{" "}
                          <span className="font-bold text-foreground/60">(Pay on Arrival)</span>
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 w-full max-w-lg">
                      <Button
                        variant="outline"
                        onClick={() => setStep(3)}
                        className="rounded-full h-14 px-8 font-bold border-muted text-muted-foreground hover:bg-muted flex items-center justify-center gap-2"
                      >
                        <ChevronLeft className="w-4 h-4" />
                        Back
                      </Button>
                      <Button
                        ...
                        className="flex-1 bg-accent text-primary hover:bg-white rounded-full h-16 sm:h-20 text-lg sm:text-2xl font-bold shadow-2xl hover:scale-105 transition-all flex items-center justify-center gap-3 sm:gap-4"
                      >
