                    ...
                    <div className="bg-white rounded-2xl p-5 sm:p-6 border border-border shadow-sm">
                      ...
                      <div className="grid md:grid-cols-2 gap-6 items-start">
                        <div className="flex flex-col gap-4">
                          {((selectedBooking as any).roomIds?.length ? (selectedBooking as any).roomIds : [selectedBooking.roomId]).map((rId: string, i: number) => {
                             const r = roomsList?.find(x => x.id === rId);
                             const img = r?.heroImageUrl || (r?.gallery && r.gallery[0]);
                             const roomName = r?.name || selectedBooking.roomName || `Room ${i + 1}`;
                             return (
                               <div key={i} className="flex gap-4 items-center bg-slate-50 p-3 rounded-2xl shadow-sm border border-slate-100">
                                 {img && (
                          ...
                          <div className="mt-1 px-2">
                            <p className="text-sm text-muted-foreground font-bold">{selectedBooking.guestCount} {selectedBooking.guestCount === 1 ? 'Guest total' : 'Guests total'}</p>
                          </div>
                        </div>
