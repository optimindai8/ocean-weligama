...
export function PackageComparisonTable() {
  ...
  return (
    <div ref={ref} className="w-full max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
      ...
      <div className="mt-12 w-full max-w-[1000px] mx-auto pb-20">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ delay: 0.5, duration: 0.7 }}
          className="bg-white rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.08)] overflow-hidden border border-slate-100"
        >
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[700px]">
              ...
              <tbody>
                {(!matrixData?.rooms || matrixData.rooms.length === 0) ? (
                  <tr>
                    <td 
                      colSpan={(matrixData?.packages?.length || 3) + 1} 
                      className="p-16 text-center text-slate-400 font-medium bg-white"
                    >
                      Please add rooms and configure packages in the admin panel to view the pricing matrix.
                    </td>
                  </tr>
                ) : (
